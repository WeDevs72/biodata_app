...
export function BusinessLivePreview({ template }: { template: string }) {
  ...
  const [prices, setPrices] = useState({ inr: 89, usd: 2.00 });
    ...
    const loadPrices = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setPrices({
            inr: settings.priceBusinessINR !== undefined ? settings.priceBusinessINR : 89,
            usd: settings.priceBusinessUSD !== undefined ? settings.priceBusinessUSD : 2.00
          });
        }
      } catch (e) {
          ...
          <div className="absolute inset-x-0 bottom-10 flex flex-col items-center justify-center p-6 text-center z-30 pointer-events-none">
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur shadow-2xl rounded-2xl p-6 max-w-sm border border-slate-200/50 dark:border-slate-800/50 pointer-events-auto transform translate-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">
                This is a preview. Download the high-quality, watermark-free PDF to unlock and view the complete profile.
              </p>
              <button
                onClick={handleDownload}
                className="w-full py-2.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-white text-sm font-bold shadow-md hover:opacity-90 active:scale-95 transition-all"
              >
